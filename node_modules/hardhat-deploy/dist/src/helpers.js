...
getArtifact, saveDeployment, willSaveToDisk, onPendingTx, getGasPrice, log, print) {
    ...
    async function _deploy(name, options) {
        ...
        const address = factory.getDeployedAddress(receipt, options, create2Address);
        const deployment = Object.assign(Object.assign({}, preDeployment), { address,
            receipt, transactionHash: receipt.transactionHash, libraries: options.libraries, factoryDeps: ((_a = unsignedTx.customData) === null || _a === void 0 ? void 0 : _a.factoryDeps) || [] });
    ...
    async function fetchIfDifferent(name, options) {
        ...
        if (deployment) {
            ...
            if (transaction) {
                const differences = await factory.compareDeploymentTransaction(transaction, deployment);
    ...
    async function _deployViaProxy(name, options) {
        ...
        if (!oldDeployment || implementation.newlyDeployed) {
            ...
            if (!proxy) {
                ...
                const deployedProxy = new contracts_1.Contract(proxy.address, proxy.abi, provider);
                if (deployedProxy.functions['owner']) {
                    ownerStorage = await deployedProxy.owner();
                }
                else {
                    ownerStorage = await provider.getStorageAt(proxy.address, '0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103');
                }
